...
An autonomous AI research agent executing multi-step search query decomposition, source evidence collection, and comprehensive executive brief synthesis.

---

## Key Features

- **Dynamic**: research goal decomposition into atomic search queries
- **Multi-source**: evidence gathering and ranking algorithms
- **Automated**: contradiction detection between competing information sources
- **Comprehensive**: executive report generation formatted in clean Markdown
- **Pluggable**: search backends with offline synthesis fallback
...
```mermaid
flowchart TD
    Goal([Research Topic]) --> Agent[AI Research Agent]
    Agent --> Decompose[Query Planner]
    Decompose --> Search[Search Provider]
    Search --> FactStore[(Evidence Store)]
    FactStore --> Synthesizer[Executive Brief Generator]
    Synthesizer --> Report[Comprehensive Research Report]
```
...
|---|---|---|
| `GET` | `/` | Research agent service health |
| `POST` | `/research` | Execute multi-step autonomous research query |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/research -H "Content-Type: application/json" -d '{"topic": "Quantum Computing Market Trends 2026", "depth": "detailed"}'
```
